...
type ExpenceState = {
  ...
  getBudget: (month?: string) => Promise<void>;
  getDashboard: (month?: string) => Promise<void>;
...
export const useExpenseStore = create<ExpenceState>((set) => ({
  ...
  getBudget: async (month?: string) => {
    try {
      ...
      const response = await axiosInstance.get(`/expense/get-budget${params}`);
  ...
  getHistory: async (month?: string) => {
    try {
      const params = month ? `?month=${month}` : "";
      const response = await axiosInstance.get(`/expense/history${params}`);
      const data = response.data?.data || response.data;
